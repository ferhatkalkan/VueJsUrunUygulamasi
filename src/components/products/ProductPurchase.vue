        ...
        <div class="row">
            <div class="col-6 offset-3 pt-3 card mt-5 shadow">
                <div class="card-body">
                    ...
                    <div class="form-group">
                        <label>Adet</label>
                        <input v-model="product.count" type="text" class="form-control"
...
export default {
    ...
    beforeRouteLeave(to, from, next) { // bu route'dan çıkmak istediğniz zaman, otomatik devreye girer.
        ...
        if ((this.product.title.length > 0 || this.product.count > 0 || this.product.price > 0 || this.product.description.length > 0) && !this.saveButtonClicked) {
            if (confirm("Kaydedilmemiş değişikliklerin var. Yine de çıkmak istiyor musunuz?")) {
                next();
            }
            else {
                next(false);
